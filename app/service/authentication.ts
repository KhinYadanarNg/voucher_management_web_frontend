...
export async function registerUser(email: string, username: string, password: string, role: string, image?: File) {
    ...
    try {
        const response = await fetch(
            `${process.env.serverURL}/api/user/create`,
            {
                method: 'POST',
                body: formData
...
export async function forgotPassword(email: string, password: string) {
    ...
    const response = await fetch(
        `${process.env.serverURL}/api/user/resetPassword`,
        {
            method: 'POST',
            headers: headers,
...
export async function loginUser(email: string, password: string) {
    ...
    const response = await fetch(
        `${process.env.serverURL}/api/user/login`,
        {
            method: 'POST',
            headers: headers,
            body: JSON.stringify(body)
...
export const verifyUser = async (verifyId: string) => {
    ...
    const res = await fetch(
        `${process.env.serverURL}/api/user/verify?verifyid=${verifyId}`,
        {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
